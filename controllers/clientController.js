...
const clientController = {
  ...
  createClient: async (req, res) => {
    try {
      const {
        name, email, phone, address, account_number
      } = req.body;
      
      if (!name || !email || !account_number) {
        return res.status(400).json({ message: 'Required fields missing: name, email, and account_number are required' });
      }
      
      const [result] = await db.query(
        `INSERT INTO clients (name, email, phone, address, account_number) VALUES (?, ?, ?, ?, ?)`,
        [name, email, phone || null, address || null, account_number]
      );
